...
## 👨‍🏫 프로젝트 개요
  ...
  ## :alarm_clock: 문제인식
  #### 우리나라의 주식투자자 수는 계속 증가해오고 있습니다. 반면 우리나라의 금융 자산 비율은 35% 정도로 약 70%의 미국에 비해 매우 작은 상태입니다. 하지만 빠르게 발전하는 국가 경제와는 다르게 개인의 경제 발전 속도는 빈익빈 부익부의 고도화에 따라 매년 늦어지는 추세입니다. 또한 다른 자산들은 시드머니가 매우 큰 반면에 주식은 상대적으로 적은 시드 머니로도 투자가 가능하기 때문에 이러한 국내 상황에서 가장 좋은 투자 방법은 금융 투자가 되어가고 있습니다. 그러나 국내 개인투자자 대부분은 바쁜 일상에 치여 제대로된 증권 정보를 얻기가 쉽지 않은 실정입니다. 증가하고 있는 주식 투자 비율의 위험도를 낮추기 위해서는 정보를 효과적으로 습득하는 것이 중요합니다. 따라서 이를 위한 도구에 대한 수요는 앞으로도 증가할 예정입니다. 우리는 이러한 사람들의 니즈를 만족시키기 위해  실시간 증권 시황과 뉴스 정보를 효과적으로 제공하는 챗봇을 구현하려 합니다.


## 👩‍🏫 서비스 목표
  #### 바쁘디 바쁜 현대 사회를 살아가는 가운데 편리하고 빠르게 주식에 대한 정보를 손쉽게 알게 되기 위해 이 LLM 모델을 구성하였습니다.
  #### 서비스는 10분마다 업데이트를 하여 최신의 정보를 사용자에게 제공해 주려고 합니다.
...
## ✈️ 향후 계획 및 개선점
 ...
 #### Private한 정보로는 이 모델을 조금 더 고도화 시킴으로 인해서 레포트로 작성을 하는 부분으로 확장할 예정입니다.
 ### 🧰
